...
class CourseRegisterStudentService {
    ...
    async execute(token: tokenInput): Promise<void> {
        try {
            await this.laravelApi.post(`/course_unregister/${token.course_id}`, 
...
@Controller('course-unregister')
class CourseUnregisterStudentController {
...
@Module({
    ...
    controllers: [ CourseUnregisterStudentController],
})
export class CourseUnregisterStudentModule{}
